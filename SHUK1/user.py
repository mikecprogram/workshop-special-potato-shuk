...
class user:
    ...
    def register(self, username, password):
        if type(self._state).__name__ == "guest":
            self._state.register(self.market._id, username, password)
        else:
            print("already logged in")

    def login(self, username, password):
        if type(self._state).__name__ == "guest":
            if (self._state.login(self.market._id, username, password)):
    ...
    def getShops(self):
        return self.market.getShops()

    def getShopDetails(self, shopname):
        return self.market.getShopDetails(shopname)

    def search(self, name=None, category=None, keyword=None, maxPrice=None, minItemRating=None, minShopRating=None):
        return self.market.search(name, category, keyword, maxPrice, minItemRating, minShopRating)
    ...
    def commitPurchase(self):
        if self.market.commitPurchase(self._shoppingCart):
            self._shoppingCart = shoppingCart(self)
